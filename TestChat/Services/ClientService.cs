...
namespace TestChat.Services
{
    public class ClientService 
    {
        ...
        public async Task Connect(ClientWebSocket socket, string Connection)
        {
            await socket.ConnectAsync(new Uri(Connection), CancellationToken.None);
        }

        public async Task Disconnect(ClientWebSocket socket, string message)
        {
            await socket.CloseAsync(WebSocketCloseStatus.NormalClosure, message, CancellationToken.None);
        }
        public async Task Send(ClientWebSocket socket, ChatMessage message)
        {
